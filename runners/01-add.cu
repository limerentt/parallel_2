...
int main(int argc, char** argv) {
    ...
    std::cout << "Hello, Parallel and Distributed World!" << std::endl;
    ...
    for (int i = 0; i < VECTOR_SIZE_CNT; ++i) {
        h_x[i] = float(i % 10) / 100;
        h_y[i] = float(i % 50) / 500;
    }
	...
	int GRID_SIZE = (VECTOR_SIZE_CNT + BLOCK_SIZE - 1) / BLOCK_SIZE;
	KernelAdd<<<GRID_SIZE, BLOCK_SIZE>>>(VECTOR_SIZE_CNT, d_x, d_y, d_result);
